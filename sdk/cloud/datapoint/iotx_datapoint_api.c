...
void intoyunParseReceiveDatapoints(const uint8_t *payload, uint32_t len)
{
    ...
    if (RESULT_DATAPOINT_NEW == IOT_DataPoint_ReadBool(DPID_DEFAULT_BOOL_RESET, &dpReset)) {
        ...
        systemNotifyEvent(EVENT_DATAPOINT, NULL, 0);
    }
}
...
int IOT_DataPoint_SendCustomData(const uint8_t *buffer, uint16_t len)
{
    ...
    payloadLen = len + 1;
    pPayload = (uint8_t *)malloc(payloadLen);
    pPayload[0] = DATA_PROTOCOL_CUSTOM;
    memcpy(&pPayload[1], buffer, len);
    result = intoyunTransmitData(pPayload, payloadLen);
    free(pPayload);
    return result;
}
