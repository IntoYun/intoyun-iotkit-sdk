...
static void _guider_get_timestamp_str(char *buf, int len)
{
    ...
    if (retry > 1) {
        MOLMC_LOGE("guider", "utils_get_epoch_time_from_ntp() retry = %d.", retry);
    }

    if (ret == 0) {
        MOLMC_LOGE("guider", "utils_get_epoch_time_from_ntp() failed!");
    }
...
{
    MOLMC_LOGD("guider", "%s", "....................................................");
    MOLMC_LOGD("guider", "%20s : %-s", "product_id", pdev_info->product_id);
    MOLMC_LOGD("guider", "%20s : %-s", "product_secret", pdev_info->product_secret);
    MOLMC_LOGD("guider", "%20s : %-s", "device_id", pdev_info->device_id);
    MOLMC_LOGD("guider", "%20s : %-s", "device_secret", pdev_info->device_secret);
    MOLMC_LOGD("guider", "%20s : %-s", "hardware_version", pdev_info->hardware_version);
    MOLMC_LOGD("guider", "%20s : %-s", "software_version", pdev_info->software_version);
    MOLMC_LOGD("guider", "%s", "....................................................");
    MOLMC_LOGD("guider", "%20s : %d", "server_port", pconn_info->port);
    MOLMC_LOGD("guider", "%20s : %-s", "server_host", pconn_info->host_name);
    MOLMC_LOGD("guider", "%20s : %-s", "client_id", pconn_info->client_id);
    MOLMC_LOGD("guider", "%20s : %-s", "username", pconn_info->username);
    MOLMC_LOGD("guider", "%20s : %-s", "password", pconn_info->password);
    MOLMC_LOGD("guider", "%20s : %-s", "random", pconn_info->random_str);
    MOLMC_LOGD("guider", "%20s : %d (%s)", "SecMode", secure_mode, secmode_str[secure_mode]);
    MOLMC_LOGD("guider", "%s", "....................................................");
    return;
}
...
int iotx_guider_authenticate(void)
{
    ...
    MOLMC_LOGD("guider", "timestamp = %s", time_stamp_str);
...
int iotx_guider_auth_gen_keys(void)
{
    ...
    MOLMC_LOG_BUFFER_HEX("guider", pconn_info->appskey, 16);
    MOLMC_LOG_BUFFER_HEX("guider", pconn_info->nwkskey, 16);
    return 0;
}
